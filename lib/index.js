/**
 * dsh-memory-evolve-suite — persistent long-term memory and background memory
 ...
 * @module dsh-memory-evolve-suite
 */
...
export const name = 'dsh-memory-evolve-suite'
  ...
  switch (key) {
    ...
    case 'searchDocsMode':
      if (key === 'searchDocsMode') {
        ...
        if (value !== null && !['all', 'filename', 'content', 'off'].includes(value)) {
          throw new Error('dsh-memory-evolve-suite: searchDocsMode 必须是 all / filename / content / off 之一（或 null 恢复默认推断）')
        }
        return
      }
      if (typeof value !== 'boolean') throw new Error(`dsh-memory-evolve-suite: ${key} 必须是布尔值`)
      return
    case 'coiEnabled':
    ...
    case 'reviewInterval':
      if (typeof value !== 'number' || !Number.isFinite(value) || value < 1) {
        throw new Error('dsh-memory-evolve-suite: reviewInterval 必须 >= 1')
      }
      return
    case 'writeGuardThreshold':
      if (typeof value !== 'number' || !Number.isInteger(value) || value < 1) {
        throw new Error('dsh-memory-evolve-suite: writeGuardThreshold 必须是 >= 1 的整数')
      }
      return
    case 'reviewMode':
      if (value !== 'suggest' && value !== 'auto') throw new Error('dsh-memory-evolve-suite: reviewMode 必须是 "suggest" 或 "auto"')
      return
    case 'advisorEnabled':
    ...
    case 'advisorModel':
      if (value !== null && (typeof value !== 'string' || value.trim() === '')) {
        throw new Error(`dsh-memory-evolve-suite: ${key} 必须是 null（继承会话模型）或非空字符串`)
      }
      return
    case 'advisorSystemPrompt':
      if (typeof value !== 'string') throw new Error('dsh-memory-evolve-suite: advisorSystemPrompt 必须是字符串')
      if (value.length > 8192) throw new Error('dsh-memory-evolve-suite: advisorSystemPrompt 超长（上限 8192 字符）')
      return
    case 'advisorImmuneTurns':
    case 'advisorMaxMessages':
    case 'advisorMaxQueued':
      if (typeof value !== 'number' || !Number.isInteger(value) || value < 0) {
        throw new Error(`dsh-memory-evolve-suite: ${key} 必须是非负整数`)
      }
      return
    case 'advisorCallTimeoutMs':
      if (typeof value !== 'number' || !Number.isInteger(value) || value < 1) {
        throw new Error('dsh-memory-evolve-suite: advisorCallTimeoutMs 必须是正整数')
      }
      return
    case 'advisorSteerSeverities':
      if (!Array.isArray(value) || value.length === 0 || !value.every((s) => s === 'nit' || s === 'concern' || s === 'blocker')) {
        throw new Error('dsh-memory-evolve-suite: advisorSteerSeverities 必须是非空数组且元素为 nit/concern/blocker')
      }
      return
    case 'keyProgressiveDisclosure':
      if (value !== 'auto' && value !== 'off' && value !== 'on') {
        throw new Error('dsh-memory-evolve-suite: keyProgressiveDisclosure 必须是 "auto" / "off" / "on"')
      }
      return
    case 'keyFullInjectThreshold':
    case 'keyFullInjectCharLimit':
      if (typeof value !== 'number' || !Number.isInteger(value) || value < 1) {
        throw new Error(`dsh-memory-evolve-suite: ${key} 必须是正整数`)
      }
      return
    default:
      throw new Error(`dsh-memory-evolve-suite: 不可运行的配置项 "${key}"`)
  }
}
...
export function resolveConfig(raw) {
  if (raw === null || typeof raw !== 'object' || Array.isArray(raw)) {
    throw new Error('dsh-memory-evolve-suite: 配置必须是对象')
  }
  const config = { ...DEFAULTS }
  for (const [key, value] of Object.entries(raw)) {
    if (value === undefined) continue
    if (!(key in DEFAULTS)) throw new Error(`dsh-memory-evolve-suite: 未知配置项 "${key}"`)
    config[key] = value
  }
  ...
  for (const key of POSITIVE_NUMBER_KEYS) {
    ...
    if (typeof value !== 'number' || !Number.isFinite(value) || value <= 0) {
      throw new Error(`dsh-memory-evolve-suite: ${key} 必须是正数`)
    }
  }
  ...
  for (const key of BOOLEAN_KEYS) {
    if (typeof config[key] !== 'boolean') {
      throw new Error(`dsh-memory-evolve-suite: ${key} 必须是布尔值`)
    }
  }
  for (const key of STRING_KEYS) {
    if (typeof config[key] !== 'string' || config[key].length === 0) {
      throw new Error(`dsh-memory-evolve-suite: ${key} 必须是非空字符串`)
    }
  }
  if (config.reviewMode !== 'suggest' && config.reviewMode !== 'auto') {
    throw new Error('dsh-memory-evolve-suite: reviewMode 必须是 "suggest" 或 "auto"')
  }
  ...
  if (!Number.isInteger(config.advisorMaxMessages) || config.advisorMaxMessages < 0) {
    throw new Error('dsh-memory-evolve-suite: advisorMaxMessages 必须是非负整数（0=无上限）')
  }
  if (!Number.isInteger(config.advisorMaxQueued) || config.advisorMaxQueued < 1) {
    throw new Error('dsh-memory-evolve-suite: advisorMaxQueued 必须是正整数')
  }
  if (!Number.isInteger(config.advisorImmuneTurns) || config.advisorImmuneTurns < 0) {
    throw new Error('dsh-memory-evolve-suite: advisorImmuneTurns 必须是非负整数')
  }
  if (typeof config.advisorSystemPrompt !== 'string' || config.advisorSystemPrompt.length > 8192) {
    throw new Error('dsh-memory-evolve-suite: advisorSystemPrompt 必须是字符串（≤8192 字符）')
  }
  // Q1（第一轮优化）：info 注入布尔
  if (typeof config.advisorInfoInject !== 'boolean') {
    throw new Error('dsh-memory-evolve-suite: advisorInfoInject 必须是布尔值')
  }
  ...
  for (const key of ['advisorProvider', 'advisorModel']) {
    if (config[key] !== null && (typeof config[key] !== 'string' || config[key].trim() === '')) {
      throw new Error(`dsh-memory-evolve-suite: ${key} 必须是 null 或非空字符串`)
    }
  }
  ...
  if (!Array.isArray(config.searchDocsExts) || config.searchDocsExts.length === 0
    ...
    throw new Error('dsh-memory-evolve-suite: searchDocsExts 必须是非空扩展名数组（如 ["md","docx"]）')
  }
  ...
  if (config.coiNotifyCommand !== null && (typeof config.coiNotifyCommand !== 'string' || config.coiNotifyCommand.trim() === '')) {
    throw new Error('dsh-memory-evolve-suite: coiNotifyCommand 必须是字符串或 null')
  }
  if (config.searchDocsProviders !== 'auto'
    ...
    throw new Error('dsh-memory-evolve-suite: searchDocsProviders 必须是 "auto" 或非空 provider 名数组')
  }
  return config
}
...
export function apply(ctx, rawConfig = {}) {
  ...
  }, 'dsh-memory-evolve-suite: locale watcher')
  ...
  const reviewCtrl = {
    sync(desired = runtime.reviewEnabled === true) {
      ...
      if (enabled && reviewToolsDispose === null) {
        ...
        }, 'dsh-memory-evolve-suite: review tools')
  ...
  const applyRuntimePatch = (patch) => {
    ...
    try {
      ...
      try {
        ...
        throw new AggregateError([error, rollbackError], 'dsh-memory-evolve-suite: 运行时设置失败且审查工具回滚不完整')
      }
      throw error
    }
  ...
  if (config.injectMemory) {
    ...
    }, 'dsh-memory-evolve-suite: memory snapshot')
  }

  // 2. The memory tool (always registered; subagent writes are gated).
  ctx.effect(() => ctx.tools.register(memoryTool(ctx, config, store, queue, getRuntime, archive, writeGap)), 'dsh-memory-evolve-suite: memory tool')
  ...
  ctx.effect(() => ctx.tools.register(skillManageTool(ctx, config)), 'dsh-memory-evolve-suite: skill tool')
  ...
  ctx.inject(['webServer'], (webCtx) => {
    ...
    webCtx.effect(() => installApi(webCtx, {
      ...
      updateOps,
    }), 'dsh-memory-evolve-suite: web api')
  })
