...
export function installCoi(ctx, config, deps) {
  ...
  if (config.coiSyncSkills !== false) {
    try {
      ...
      if (changed.length > 0) {
        console.log(`[dsh-memory-evolve-suite] COI 内置技能已同步：${changed.map((s) => s.name).join(', ')}`)
      }
    } catch (error) {
      console.warn(`[dsh-memory-evolve-suite] COI 内置技能同步失败（忽略）：${error.message}`)
    }
  }
  ...
  }, 'dsh-memory-evolve-suite: coi tools'))
  ...
  ctx.inject(['webServer'], (webCtx) => {
    webCtx.effect(() => {
      ...
      disposers.push(d)
    }, 'dsh-memory-evolve-suite: coi web api')
  })

  // 卸载清理：释放全部定时器/进程句柄
  ctx.effect(() => () => scheduler.dispose(), 'dsh-memory-evolve-suite: coi scheduler')
...
export function installBroadcast(ctx, config) {
  ...
  const deliver = (sessionId, text, wake = false) => {
    ...
    const source = { kind: 'plugin', plugin: 'dsh-memory-evolve-suite', form: 'notice', summary: text.split('\n')[0].slice(0, 80) }
  ...
  const notifyRecipients = (msg, wake = false) => {
    ...
    for (const sid of targets) {
      try {
        ...
        failed += 1
        console.warn(`[dsh-memory-evolve-suite] 广播投递失败（接收方 ${sid}，忽略）: ${String(error)}`)
      }
    }
    return { woken, failed }
  }
  ...
  disposers.push(ctx.effect(() => {
    ...
    return () => d?.()
  }, 'dsh-memory-evolve-suite: broadcast tool'))
  ...
  disposers.push(ctx.effect
    ? ctx.effect(() => ctx.on('agent/status', ({ agent }) => {
      ...
      }), 'dsh-memory-evolve-suite: broadcast unread catchup')
    : () => {})
  ...
  ctx.inject(['webServer'], (webCtx) => {
    webCtx.effect(() => {
      ...
      disposers.push(d)
    }, 'dsh-memory-evolve-suite: broadcast web api')
  })
