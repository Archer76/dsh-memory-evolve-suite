...
function userMessage(text) {
  return {
    ...
    source: { kind: 'plugin', plugin: 'dsh-memory-evolve-suite' },
  }
}
...
export function installWsCoord(ctx, config, deps = {}) {
  ...
  disposers.push(ctx.effect
    ? ctx.effect(() => ctx.on('agent/status', ({ agent, status }) => {
      ...
      }), 'dsh-memory-evolve-suite: ws-coord agent/status')
    : () => {})
  ...
  if (config.wsCoordAutoRegister !== false) {
    disposers.push(ctx.effect
      ? ctx.effect(() => ctx.on('fs/observed', (target, _version, actor) => {
        ...
        }), 'dsh-memory-evolve-suite: ws-coord fs/observed')
      : () => {})
  }
  ...
  disposers.push(ctx.effect
    ? ctx.effect(() => ctx.on('tools/pre-execute', async (exec, next) => {
      ...
      }), 'dsh-memory-evolve-suite: ws-coord pre-execute')
    : () => {})
  ...
  disposers.push(ctx.effect
    ? ctx.effect(() => ctx.on('tools/post-execute', async (exec, result, next) => {
        ...
        return decision
      }), 'dsh-memory-evolve-suite: ws-coord post-execute')
    : () => {})
  ...
  disposers.push(ctx.effect
    ? ctx.effect(() => ctx.on('agent/disposed', (payload) => {
      ...
      }), 'dsh-memory-evolve-suite: ws-coord agent/disposed')
    : () => {})
  ...
  disposers.push(ctx.effect
    ? ctx.effect(() => {
      ...
      }, 'dsh-memory-evolve-suite: ws-coord tools')
    : () => {})
  ...
  const checkAndNotify = () => {
    ...
    for (const [sessionId, meta] of sessionMeta) {
      ...
      const message = {
        ...
        source: {
          kind: 'plugin',
          plugin: 'dsh-memory-evolve-suite',
          form: 'notice',
          summary: text.split('\n')[0].slice(0, 80),
        },
      }
