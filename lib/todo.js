/**
 * dsh-memory-evolve-suite — todo store and `dtodo` tool (four tracks).
 *
 ...
 *
 * @module dsh-memory-evolve-suite/todo
 */
...
export class TodoStore {
  ...
  fileOf(target, cwd, date) {
    switch (target) {
      ...
      case 'project': {
        if (!cwd) throw new Error('dsh-memory-evolve-suite: project todo 需要会话工作目录')
      ...
      default:
        throw new Error(`dsh-memory-evolve-suite: 无效的待办轨 "${target}"`)
    }
  }
