/**
 * dsh-memory-evolve-suite — 会话书签（第一阶段：标记 + 列表 + 跳转）。
 *
 ...
 *
 * @module dsh-memory-evolve-suite/bookmarks
 */
...
export async function forkSession(ctx, options) {
  ...
  if (typeof cwd === 'string' && workspaceRegistry) {
    try {
      ...
      console.warn(`[dsh-memory-evolve-suite] 分支会话 ${childId} 挂接工作区失败（忽略）: ${String(error)}`)
    }
  }
...
export function installBookmarks(ctx, config) {
  ...
  ctx.inject(['webServer'], (webCtx) => {
    ...
    }), 'dsh-memory-evolve-suite: bookmarks route')
  })
