/**
 * dsh-memory-evolve-suite — memory storage layer.
 *
 ...
 *
 * @module dsh-memory-evolve-suite/store
 */
...
export function withLock(dir, fn) {
  ...
  for (;;) {
    ...
    if (Date.now() >= deadline) {
      throw new Error('dsh-memory-evolve-suite: timed out waiting for the memory lock')
    }
    sleep(LOCK_RETRY_MS)
  }
  heldLocks.add(dir)
...
export class MemoryStore {
  ...
  locate(target, agent) {
    switch (target) {
      ...
      default:
        throw new Error(`dsh-memory-evolve-suite: 无效的记忆轨 "${target}"`)
    }
  }
  ...
  resolveTarget(target, agent) {
    ...
    if (!loc) {
      throw new Error(`dsh-memory-evolve-suite: 无法定位记忆轨 "${target}"（项目记忆需要有效的会话工作目录）`)
    }
    return loc
  }
...
export class ArchiveStore {
  ...
  fileOf(target, cwd) {
    ...
    if (target === 'key') {
      if (!cwd) throw new Error('dsh-memory-evolve-suite: key 归档需要会话工作目录')
    ...
    throw new Error(`dsh-memory-evolve-suite: 无效的归档轨 "${target}"`)
  }
