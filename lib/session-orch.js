...
export class SessionOrchStore {
  ...
  #load() {
    try {
      ...
      if (error.code !== 'ENOENT') {
        console.warn(`[dsh-memory-evolve-suite] 会话编排记录加载失败（忽略）: ${error.message}`)
      }
    }
  }
  ...
  #save() {
    try {
      ...
      console.warn(`[dsh-memory-evolve-suite] 会话编排记录保存失败（忽略）: ${error.message}`)
    }
  }
...
export class SessionOrch {
  ...
  async attachWorkspace(sessionId, cwd) {
    ...
    console.warn(`[dsh-memory-evolve-suite] 会话 ${sessionId} 挂接工作区失败（已重试 ${this.attachDelays.length} 次，仅影响左侧分组显示，不影响会话本身）: ${lastErr}`)
    return {
      ok: false,
  ...
  async spawn({ prompt, cwd, roomId, model, provider, agentPreset, by, requester } = {}) {
    ...
    try {
      ...
      try {
        ...
        console.warn(`[dsh-memory-evolve-suite] agentPresets 解析失败（spawn 降级不挂载预设）: ${errText(presetError)}`)
      }
  ...
  async wake({ sessionId, prompt } = {}) {
    ...
    if (agent === undefined) {
      ...
      try {
        ...
        console.warn(`[dsh-memory-evolve-suite] agentPresets 解析失败（wake 恢复降级不挂载预设）: ${errText(presetError)}`)
      }
      try {
...
export function installSession(ctx, config, deps) {
  ...
  try {
    ...
    disposers.push(ctx.effect(() => {
      ...
      return () => d?.()
    }, 'dsh-memory-evolve-suite: session tool'))
  } catch (error) {
    // 理论上不会发生（agents 声明式注入保证就绪）；防御兜底：不崩插件，
    // 工具不注册，调用时报"未就绪"（execute 顶部有检查）
    console.warn(`[dsh-memory-evolve-suite] 会话编排初始化失败（de_session 未注册）: ${error.message}`)
  }
  return {
