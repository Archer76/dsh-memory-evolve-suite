/**
 * dsh-memory-evolve-suite — 版本检测与更新模块（一期，v3）。
 *
 ...
 *
 * @module dsh-memory-evolve-suite/update
 */
...
const PRIVATE_REFS = 'refs/remotes/dsh-memory-evolve-suite-update'
/** 状态文件名（检测缓存）与事务文件名（重启提示/审计，与检测分离）。 */
const STATE_FILE = 'update-state.json'
const TX_FILE = 'update-tx.json'
/** fallback 标记文件名（持久化 fallback 选择，重启后延续，P2-2）。 */
const FALLBACK_MARK_FILE = 'dsh-memory-evolve-suite-fallback.json'
...
export async function acquireUpdateLock(gitDir, now = Date.now()) {
  const lockDir = join(gitDir, 'dsh-memory-evolve-suite')
...
export function createUpdateChecker(opts = {}) {
  ...
  function pathsOf(dir) {
    return {
      cache: join(dir, 'dsh-memory-evolve-suite', STATE_FILE),
      tx: join(dir, 'dsh-memory-evolve-suite', TX_FILE),
    }
  }
