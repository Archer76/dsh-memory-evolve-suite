/**
 * dsh-memory-evolve-suite — search_local_docs 本地文档/文件检索。
 *
 ...
 *
 * 零运行时依赖（仅 node 内置模块）。
 * @module dsh-memory-evolve-suite/search-docs
 */
    ...
    async search({ query, exts, dir, limit, kind = 'file' }, { signal }) {
      ...
      if (stale.length > 0 && !scanPromise) {
        ...
        scanPromise = rebuild(stale, undefined).catch((error) => {
          scanPromise = null
          console.warn(`[dsh-memory-evolve-suite] 文档索引后台重建失败（忽略，下次查询重试）：${error?.message ?? error}`)
        })
      }
