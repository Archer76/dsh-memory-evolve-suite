...
- **架构（方案 A：渠道插件全局注册表，用户拍板）**：渠道插件（dsh-feishu 等**公共插件**）在 apply 时把主动发送能力登记到 `globalThis.__dshChannelNotify`（无侵入钩子）；本模块工具执行时读取调用，**零依赖**渠道插件：
  ...
  - 渠道插件侧改造契约见 dsh-feishu 的 README「与 dsh-memory-evolve-suite 的集成」节。
...
dsh plugin --profile web add link:/path/to/dsh-memory-evolve-suite
```
...
重启 `dsh web` 即生效。卸载：`dsh plugin --profile web remove dsh-memory-evolve-suite`，
一切效果随插件卸载自动清理。
...
```sh
mkdir -p ~/.dsh/plugins
cp -r dsh-memory-evolve-suite ~/.dsh/plugins/
ln -s ~/.dsh/plugins/dsh-memory-evolve-suite ~/node_modules/@dsh-local/dsh-memory-evolve-suite
```
...
- insert:
    - id: dsh-memory-evolve-suite
      name: '@dsh-local/dsh-memory-evolve-suite'
      config:
        reviewEnabled: true
        reviewInterval: 10
```
...
```sh
cd dsh-memory-evolve-suite && node --test 'tests/*.test.js'
```
